...
func UpdateProduct(userId string, product ds.ProductUpdated) error {
	...
	result, err := txn.Exec("update PRODUCTS set title = ?, price = ?, description = ?, updateTime = ? where productId = ? and userId = ?",
		product.Title, product.Price, product.Content, time.Now(), product.ProductId, userId)
	if err != nil {
		return NewErrorDao(ErrTypeDatabaseExec, err.Error())
	}
	affected, err := result.RowsAffected()
